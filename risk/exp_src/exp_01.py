...
specs.set_number_of_runs(100)
# set random seeds
specs.set_start_seeds(2000, 6000)

# danger specs
# true danger file
true_file = 'danger_map_100'
...
for per in list_per:
    estimated_file = 'danger_map_' + str(per)
    # estimated danger file
    specs.set_danger_file(estimated_file, 'hat')
    ...
    for turn in specs.list_turns:
        ...
        # run simulator
        belief, target, team, solver_data, danger = sr.run_simulator(specs)

        # save everything as a pickle file
        bf.save_sim_data(belief, target, team, solver_data, danger, specs)
